  ...
  if (wifiDev != nullptr) {
    ...
    if (staMac != nullptr) {
      ...
      if (pTargetNode != NULL) {
        ...
        Ptr<Node> server = NodeList::GetNode(12);
        ndn::KiteMsServer* app = dynamic_cast<ndn::KiteMsServer*>(&(*server->GetApplication(0)));
        std::string prefix = "/router/" + std::to_string(i);
...
main(int argc, char* argv[])
{
  ...
  // p2p.Install(nodes.Get(11), nodes.Get(0)); // consumer <-> 0
  p2p.Install(nodes.Get(11), nodes.Get(8)); // consumer <-> 8
  p2p.Install(nodes.Get(12), nodes.Get(1)); // rv <-> 1
  ...
  ndn::AppHelper serverHelper("ns3::ndn::KiteMsConsumerUpload");
